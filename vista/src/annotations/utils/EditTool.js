...
class EditTool extends Component {
  ...
  onMouseDrag(e) {
    ...
    paths.forEach((path) => {
      if (path.data.state === "moving") {
        ...
        const scale =
          e.point.subtract(bounds.center).length / path.data.scaleBase.length;
        const tlVec = bounds.topLeft.subtract(bounds.center).multiply(scale);
        const brVec = bounds.bottomRight
          .subtract(bounds.center)
          .multiply(scale);
        const newBounds = new Rectangle(
          tlVec.add(bounds.center),
          brVec.add(bounds.center)
        );
  ...
  onMouseMove(e) {
    ...
    paths.forEach((path) => {
      if (
        path.contains(e.point) ||
        path.hitTest(e.point, { segments: true, tolerance: 15 })
      ) {
        const hitTest = path.hitTest(e.point, {
          segments: true,
          tolerance: 15,
        });
        let cursor = "move";
